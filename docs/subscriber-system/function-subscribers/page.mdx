# Function Subscribers

Function subscribers intercept decorated service execution around persistence work. UPDATE includes an ordinary decorated GET before `onBeforeUpdate`, and transactional execution may also emit post-commit or post-rollback hooks after the outer owner finishes. They are ideal for persistence-aware validation, transformation, enrichment, auditing, and confirmed-transaction side effects.

## Creating a Function Subscriber

Extend `ApiFunctionSubscriberBase` and decorate with `@ApiFunctionSubscriber`:

```typescript filename="post-slug.subscriber.ts" copy
import { Injectable } from "@nestjs/common";
import { ApiFunctionSubscriber, ApiFunctionSubscriberBase, TApiSubscriberFunctionBeforeCreateContext, TApiFunctionCreateProperties } from "@elsikora/nestjs-crud-automator";
import { PostEntity } from "./post.entity";

@Injectable()
@ApiFunctionSubscriber({
	entity: PostEntity,
	priority: 10, // Higher priority executes first
})
export class PostSlugSubscriber extends ApiFunctionSubscriberBase<PostEntity> {
	// Implement hook methods using helper types
}
```

## Available Hooks

### Before Hooks

Called before repository operations:

#### `onBeforeCreate`

```typescript
async onBeforeCreate(
  context: TApiSubscriberFunctionBeforeCreateContext<PostEntity>
): Promise<TApiFunctionCreateProperties<PostEntity>> {
  // Fully typed access to DATA
  const manager = context.DATA.eventManager;
  const repository = context.DATA.repository;

  // Generate slug from title
  if (context.result.title && !context.result.slug) {
    context.result.slug = this.generateSlug(context.result.title);
  }

  // Set defaults
  context.result.status = context.result.status || "draft";
  context.result.viewCount = 0;

  return context.result;
}

private generateSlug(title: string): string {
  return title
    .toLowerCase()
    .replace(/[^\w\s-]/g, "")
    .replace(/\s+/g, "-");
}
```

For generated create flows that have a stricter application-defined service payload than `DeepPartial<E>`, pass that payload as the second generic. Define the payload type yourself to match the service input shape; the generic only narrows TypeScript's `context.result` type and does not change validation, transformation, or runtime behavior.

```typescript
type PostCreateInput = {
	title: string;
	content: string;
	author: { id: string };
};

export class PostCreateSubscriber extends ApiFunctionSubscriberBase<PostEntity, PostCreateInput> {
	async onBeforeCreate(context: TApiSubscriberFunctionBeforeCreateContext<PostEntity, PostCreateInput>): Promise<PostCreateInput> {
		context.result.title; // string
		context.result.author.id; // string

		return context.result;
	}
}
```

#### `onBeforeUpdate`

```typescript
async onBeforeUpdate(
  context: TApiSubscriberFunctionBeforeUpdateContext<PostEntity>
): Promise<TApiFunctionUpdateProperties<PostEntity>> {
  const currentPost = context.DATA.currentEntity; // Readonly<PostEntity>

  // Update slug if title changed
  if (context.result.title) {
    context.result.slug = this.generateSlug(context.result.title);
  }

  // Track modification count
  context.result.modificationCount = (currentPost.modificationCount || 0) + 1;

  return context.result;
}
```

Automator loads `currentEntity` with one ordinary decorated GET before invoking `onBeforeUpdate`. Only the snapshot's top-level object is detached and frozen; nested values retain aliases to the internal loaded entity and can affect persistence if mutated. `context.result` remains the mutable update patch. When a transaction is active, both the GET and `context.DATA.repository` use its manager. A missing entity runs GET error hooks, skips `onBeforeUpdate`, and then runs UPDATE error hooks.

#### `onBeforeGet`, `onBeforeGetList`, `onBeforeGetMany`, `onBeforeDelete`

```typescript
async onBeforeGet(
  context: TApiSubscriberFunctionBeforeGetContext<PostEntity>
): Promise<TApiFunctionGetProperties<PostEntity>> {
  // Fully typed access to repository
  const repository = context.DATA.repository;

  // Inspect the requested query before fetching
  await repository.count({
    where: context.result.where,
  });

  return context.result;
}

async onBeforeDelete(
  context: TApiSubscriberFunctionBeforeDeleteContext<PostEntity>
): Promise<TApiFunctionDeleteCriteria<PostEntity>> {
  // Archive before deletion using delete criteria
  await this.archiveService.archive(context.result);

  return context.result;
}
```

### Transaction Expectations

Function subscribers can declare that they require a transaction manager. This is a subscriber-level expectation and is not inferred from the service or function transaction config.

When `transaction.expectation` is `REQUIRED` or `MANDATORY`, pass the same expectation to `ApiFunctionSubscriberBase` and the helper context type. `context.DATA.eventManager` is then typed as `EntityManager`, and the executor throws before your hook runs if no manager is available. The generic is type-only; decorator metadata is the runtime source of truth.

```typescript
import { ApiFunctionSubscriber, ApiFunctionSubscriberBase, EApiFunctionSubscriberTransactionExpectation, TApiSubscriberFunctionBeforeCreateContext, TApiFunctionCreateProperties } from "@elsikora/nestjs-crud-automator";

@ApiFunctionSubscriber({
	entity: PostEntity,
	transaction: { expectation: EApiFunctionSubscriberTransactionExpectation.REQUIRED },
})
export class PostAuditSubscriber extends ApiFunctionSubscriberBase<PostEntity, TApiFunctionCreateProperties<PostEntity>, EApiFunctionSubscriberTransactionExpectation.REQUIRED> {
	async onBeforeCreate(context: TApiSubscriberFunctionBeforeCreateContext<PostEntity, TApiFunctionCreateProperties<PostEntity>, EApiFunctionSubscriberTransactionExpectation.REQUIRED>): Promise<TApiFunctionCreateProperties<PostEntity>> {
		await context.DATA.eventManager.save(AuditEntity, {
			postTitle: context.result.title,
		});

		return context.result;
	}
}
```

`SUPPORTS`, `NONE`, and subscribers without `transaction` metadata keep the default `EntityManager | undefined` behavior.

### After Hooks

Called after repository operations:

#### `onAfterCreate`

```typescript
async onAfterCreate(
  context: TApiSubscriberFunctionAfterCreateContext<PostEntity>
): Promise<PostEntity> {
  const post = context.result;

  // Create related entities
  if (post.tags) {
    await this.createTags(post.id, post.tags);
  }

  // Update counters
  await this.updateAuthorPostCount(post.authorId);

  return post;
}
```

#### `onAfterUpdate`

```typescript
async onAfterUpdate(
  context: TApiSubscriberFunctionAfterUpdateContext<PostEntity>
): Promise<PostEntity> {
  const post = context.result;

  // Update search index
  await this.searchService.updateIndex(post);

  // Regenerate sitemap
  await this.sitemapService.regenerate();

  return post;
}
```

#### `onAfterGet`, `onAfterGetList`, `onAfterGetMany`, `onAfterDelete`

```typescript
async onAfterGetList(
  context: TApiSubscriberFunctionAfterGetListContext<PostEntity>
): Promise<IApiGetListResponseResult<PostEntity>> {
  const response = context.result;

  // Enrich with computed data
  for (const post of response.items) {
    post.readingTime = this.calculateReadingTime(post.content);
  }

  return response;
}

private calculateReadingTime(content: string): number {
  const words = content.split(/\s+/).length;
  return Math.ceil(words / 200); // 200 words per minute
}
```

### Post-Transaction Hooks

`onAfterCommit` and `onAfterRollback` run once per matching subscriber after the outer transaction owner confirms the database result. They do not run for `NONE`, nontransactional `SUPPORTS`, or an unknown COMMIT outcome.

```typescript
async onAfterCommit(
	context: IApiSubscriberFunctionTransactionContext,
): Promise<void> {
	await this.metrics.recordCommittedOperations(
		context.DATA.matchedEvents.length,
	);
}

async onAfterRollback(
	context: IApiSubscriberFunctionTransactionContext,
): Promise<void> {
	await this.metrics.recordRollback(context.DATA.transaction.id);
}
```

The readonly context exposes the transaction id and `FUNCTION`, `ROUTE`, or `SCOPE` owner, all ordered events, and subscriber-matched events. Events are payload-free operation metadata; outbox bodies and domain payloads must come from application-owned state. `STEP` appears only as a trace event. Post-transaction hooks cannot return a replacement result.

Commit and rollback error lifecycle methods are:

- `onBeforeErrorCommit` and `onBeforeErrorRollback`, which receive raw collected failures;
- `onAfterErrorCommit` and `onAfterErrorRollback`, which receive the normalized transaction exception.

A post-commit hook failure produces `ApiFunctionTransactionPostCommitException` with outcome `COMMITTED`; the database work must not be represented as rolled back.

### Error Hooks

Called when repository operations fail:

```typescript
async onBeforeErrorCreate(
  context: IApiSubscriberFunctionErrorExecutionContext<PostEntity>,
  error: Error
): Promise<void> {
  console.error(`Post creation about to fail: ${FormatErrorEvidenceForLog(error)}`);
}

async onAfterErrorCreate(
  context: IApiSubscriberFunctionErrorExecutionContext<PostEntity>,
  error: Error
): Promise<void> {
  // Cleanup after error
  if (context.ENTITY.tempFiles) {
    await this.cleanupTempFiles(context.ENTITY.tempFiles);
  }
}

// Similar hooks for other operations
async onBeforeErrorUpdate(...) {}
async onAfterErrorUpdate(...) {}
async onBeforeErrorGet(...) {}
async onAfterErrorGet(...) {}
async onBeforeErrorGetList(...) {}
async onAfterErrorGetList(...) {}
async onBeforeErrorGetMany(...) {}
async onAfterErrorGetMany(...) {}
async onBeforeErrorDelete(...) {}
async onAfterErrorDelete(...) {}
```

Use the package `FormatErrorEvidenceForLog` utility for deliberate error logs. Subscriber entity/context data, messages, stacks, causes, SQL, parameters, and driver objects are not safe log evidence.

## Practical Examples

### Automatic Slug Generation

```typescript filename="slug-generator.subscriber.ts" copy
import slugify from "slugify";
import { TApiSubscriberFunctionBeforeCreateContext, TApiFunctionCreateProperties } from "@elsikora/nestjs-crud-automator";

@Injectable()
@ApiFunctionSubscriber({ entity: PostEntity })
export class SlugGeneratorSubscriber extends ApiFunctionSubscriberBase<PostEntity> {
	async onBeforeCreate(context: TApiSubscriberFunctionBeforeCreateContext<PostEntity>): Promise<TApiFunctionCreateProperties<PostEntity>> {
		if (context.result.title && !context.result.slug) {
			context.result.slug = slugify(context.result.title, {
				lower: true,
				strict: true,
				trim: true,
			});

			// Ensure uniqueness
			context.result.slug = await this.ensureUniqueSlug(context.result.slug);
		}

		return context.result;
	}

	private async ensureUniqueSlug(baseSlug: string): Promise<string> {
		let slug = baseSlug;
		let counter = 1;

		while (await this.slugExists(slug)) {
			slug = `${baseSlug}-${counter}`;
			counter++;
		}

		return slug;
	}

	private async slugExists(slug: string): Promise<boolean> {
		const count = await this.repository.count({ where: { slug } });
		return count > 0;
	}
}
```

### Full-Text Search Indexing

```typescript filename="search-index.subscriber.ts" copy
@Injectable()
@ApiFunctionSubscriber({ entity: PostEntity })
export class SearchIndexSubscriber extends ApiFunctionSubscriberBase<PostEntity> {
	constructor(
		@InjectRepository(PostEntity)
		public repository: Repository<PostEntity>,
		private readonly searchService: SearchService,
	) {
		super();
	}

	async onAfterCreate(context: TApiSubscriberFunctionAfterCreateContext<PostEntity>): Promise<PostEntity> {
		await this.searchService.indexDocument({
			id: context.result.id,
			title: context.result.title,
			content: context.result.content,
			tags: context.result.tags,
		});

		return context.result;
	}

	async onAfterUpdate(context: TApiSubscriberFunctionAfterUpdateContext<PostEntity>): Promise<PostEntity> {
		await this.searchService.updateDocument(context.result.id, {
			title: context.result.title,
			content: context.result.content,
			tags: context.result.tags,
		});

		return context.result;
	}

	async onAfterDelete(context: TApiSubscriberFunctionAfterDeleteContext<PostEntity>): Promise<PostEntity> {
		await this.searchService.deleteDocument(context.result.id);

		return context.result;
	}
}
```

### Computed Fields

```typescript filename="computed-fields.subscriber.ts" copy
@Injectable()
@ApiFunctionSubscriber({ entity: PostEntity })
export class ComputedFieldsSubscriber extends ApiFunctionSubscriberBase<PostEntity> {
	async onAfterGet(context: TApiSubscriberFunctionAfterGetContext<PostEntity>): Promise<PostEntity> {
		const post = context.result;

		// Calculate reading time
		post.readingTimeMinutes = this.calculateReadingTime(post.content);

		// Calculate content summary
		post.summary = this.generateSummary(post.content, 200);

		return post;
	}

	async onAfterGetList(context: TApiSubscriberFunctionAfterGetListContext<PostEntity>): Promise<IApiGetListResponseResult<PostEntity>> {
		for (const post of context.result.items) {
			post.readingTimeMinutes = this.calculateReadingTime(post.content);
			post.summary = this.generateSummary(post.content, 150);
		}

		return context.result;
	}

	private calculateReadingTime(content: string): number {
		const words = content.split(/\s+/).length;
		return Math.ceil(words / 200);
	}

	private generateSummary(content: string, maxLength: number): string {
		if (content.length <= maxLength) return content;
		return content.substring(0, maxLength).trim() + "...";
	}
}
```

### Cascade Operations

```typescript filename="cascade-operations.subscriber.ts" copy
@Injectable()
@ApiFunctionSubscriber({ entity: PostEntity })
export class CascadeOperationsSubscriber extends ApiFunctionSubscriberBase<PostEntity> {
	constructor(
		@InjectRepository(PostEntity)
		public repository: Repository<PostEntity>,
		@InjectRepository(CommentEntity)
		private commentRepository: Repository<CommentEntity>,
		@InjectRepository(LikeEntity)
		private likeRepository: Repository<LikeEntity>,
	) {
		super();
	}

	async onBeforeDelete(context: TApiSubscriberFunctionBeforeDeleteContext<PostEntity>): Promise<TApiFunctionDeleteCriteria<PostEntity>> {
		const criteria = context.result;

		// Delete related comments
		await this.commentRepository.delete(criteria);

		// Delete related likes
		await this.likeRepository.delete(criteria);

		return context.result;
	}
}
```

### Versioning and History

```typescript filename="version-history.subscriber.ts" copy
@Injectable()
@ApiFunctionSubscriber({ entity: PostEntity })
export class VersionHistorySubscriber extends ApiFunctionSubscriberBase<PostEntity> {
	constructor(
		@InjectRepository(PostEntity)
		public repository: Repository<PostEntity>,
		@InjectRepository(PostHistoryEntity)
		private historyRepository: Repository<PostHistoryEntity>,
	) {
		super();
	}

	async onAfterUpdate(context: TApiSubscriberFunctionAfterUpdateContext<PostEntity>): Promise<PostEntity> {
		const post = context.result;

		// Save version to history
		await this.historyRepository.save({
			postId: post.id,
			title: post.title,
			content: post.content,
			version: post.version,
			createdAt: new Date(),
		});

		// Increment version
		await this.repository.update({ id: post.id }, { version: post.version + 1 });

		return post;
	}
}
```

### Data Validation

```typescript filename="data-validation.subscriber.ts" copy
@Injectable()
@ApiFunctionSubscriber({ entity: PostEntity, priority: 100 })
export class DataValidationSubscriber extends ApiFunctionSubscriberBase<PostEntity> {
	async onBeforeCreate(context: TApiSubscriberFunctionBeforeCreateContext<PostEntity>): Promise<TApiFunctionCreateProperties<PostEntity>> {
		const body = context.result;

		// Validate content length
		if (body.content && body.content.length < 100) {
			throw new BadRequestException("Post content must be at least 100 characters");
		}

		// Validate image URLs
		if (body.images) {
			for (const imageUrl of body.images) {
				if (!(await this.isValidImageUrl(imageUrl))) {
					throw new BadRequestException(`Invalid image URL: ${imageUrl}`);
				}
			}
		}

		return context.result;
	}

	private async isValidImageUrl(url: string): Promise<boolean> {
		try {
			const response = await fetch(url, { method: "HEAD" });
			const contentType = response.headers.get("content-type");
			return contentType?.startsWith("image/") || false;
		} catch {
			return false;
		}
	}
}
```

## Priority Ordering

Function subscribers execute in priority order (high to low for both before and after hooks):

```typescript
// Validation (highest priority - executes first)
@ApiFunctionSubscriber({ entity: PostEntity, priority: 100 })
export class ValidationSubscriber {}

// Data enrichment
@ApiFunctionSubscriber({ entity: PostEntity, priority: 50 })
export class EnrichmentSubscriber {}

// Logging (lowest priority - executes last)
@ApiFunctionSubscriber({ entity: PostEntity, priority: 10 })
export class LoggingSubscriber {}
```

## Next Steps

- [Route Subscribers](/docs/nestjs-crud-automator/subscriber-system/route-subscribers) - Controller-level hooks
- [Execution Context](/docs/nestjs-crud-automator/subscriber-system/execution-context) - Context interfaces reference
- [Lifecycle](/docs/nestjs-crud-automator/subscriber-system/lifecycle) - Detailed execution flow
