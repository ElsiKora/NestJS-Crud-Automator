...
8. Function subscribers: onAfterCreate (priority: high → low)
9. Service returns result
10. Controller transformers (response)
11. Route subscribers: onAfterCreate (priority: high → low)
...
6. Resolve or join the configured function transaction
7. Function subscribers: onBeforeGet (priority: high → low)
8. Repository: findOne() through the resolved repository
9. Function subscribers: onAfterGet (priority: high → low)
10. Create detached top-level frozen currentEntity snapshot
11. Function subscribers: onBeforeUpdate (priority: high → low)
12. Merge the returned patch into the internal loaded entity
13. Repository: save() executed with updates
14. Function subscribers: onAfterUpdate (priority: high → low)
15. Commit and post-commit lifecycle when UPDATE owns the transaction
16. Service returns result
17. Controller transformers (response)
18. Route subscribers: onAfterUpdate (priority: high → low)
19. Authorization result transforms
20. Relation projection and DTO serialization
21. HTTP Response sent to client
```

If the decorated GET misses, its GET error lifecycle runs, `onBeforeUpdate` is skipped, and the UPDATE error lifecycle receives the same error.
...
6. Function subscribers: onAfterGet (priority: high → low)
7. Service returns result
8. Controller transformers (response)
9. Route subscribers: onAfterGet (priority: high → low)
10. Authorization result transforms
11. Relation projection and DTO serialization
...
7. Function subscribers: onAfterGetList (priority: high → low)
8. Service returns result with pagination metadata
9. Controller transformers (response)
10. Route subscribers: onAfterGetList (priority: high → low)
11. Authorization result transforms
12. Relation projection and DTO serialization
...
7. Function subscribers: onAfterDelete (priority: high → low)
8. Service returns result
9. Route subscribers: onAfterDelete (priority: high → low)
...
4. Errors are rethrown after subscribers observe them.
```
