...
async onBeforeCreate(
  ...
  // Before hooks have only route metadata in DATA
...
Called after successful operation. After hooks have access to extended DATA with authentication, `authorizationDecision`, headers, and IP:
