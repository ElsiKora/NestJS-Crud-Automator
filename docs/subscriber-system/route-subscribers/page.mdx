...
async onBeforeCreate(
  ...
  // Before hooks have route metadata in DATA
  // Request payload and optional authenticationRequest live in context.result
...
Before route subscribers keep `authenticationRequest` optional by default. If a subscriber is attached only to routes that guarantee authentication and authorization, declare the expectation on both the decorator metadata and the class/context generic:

```typescript
import { ApiRouteSubscriber, ApiRouteSubscriberBase, EApiRouteSubscriberAuthorizationExpectation, EApiRouteType, TApiSubscriberRouteBeforeCreateContext } from "@elsikora/nestjs-crud-automator";
...
export class PostAuditSubscriber extends ApiRouteSubscriberBase<PostEntity, EApiRouteSubscriberAuthorizationExpectation.REQUIRED> {
	async onBeforeCreate(context: TApiSubscriberRouteBeforeCreateContext<PostEntity, EApiRouteSubscriberAuthorizationExpectation.REQUIRED>): Promise<TApiSubscriberRouteBeforeCreateContext<PostEntity, EApiRouteSubscriberAuthorizationExpectation.REQUIRED>["result"]> {
		const principal = context.result.authenticationRequest.authorizationDecision.principal;

		await this.auditService.record(principal.id, context.result.body);
...
This is type-only metadata. It does not add authentication or authorization guards, and routes without this expectation keep optional access for `authenticationRequest` and `authorizationDecision`.
...
The default remains PAGE. A subscriber dedicated to CURSOR routes declares the mode on the base class and context helper:

```typescript
export class PostCursorAuditSubscriber extends ApiRouteSubscriberBase<PostEntity, EApiRouteSubscriberAuthorizationExpectation.OPTIONAL, EApiControllerGetListQueryPaginationMode.CURSOR> {
	async onAfterGetList(context: TApiSubscriberRouteAfterGetListContext<PostEntity, EApiControllerGetListQueryPaginationMode.CURSOR>): Promise<IApiGetListCursorResponseResult<PostEntity> | undefined> {
		console.log(context.result.nextCursor, context.DATA.query?.after);
...
Generated CURSOR calls allow unrelated item redaction, but route subscribers must preserve the flat envelope, both cursors, item cardinality/order, and every raw effective-order/primary tuple. Violations fail closed before the response is emitted.
...
async onBeforeErrorCreate(
  ...
  console.error(`Post creation is about to fail: ${FormatErrorEvidenceForLog(error)}`);
}
...
Error hooks may inspect route context for decisions, but raw context, request URL, headers, IP, authentication data, error messages, stacks, causes, SQL, parameters, and driver objects must not be logged.
