...
Use `IApiSubscriberRouteExecutionContextData<E>` for before-error hooks and `IApiSubscriberRouteExecutionContextDataExtended<E, R>` for after-error hooks when you need typed access to authentication data or `authorizationDecision`.
...
Route before hooks receive request payloads (`body`, `parameters`, `query`) rather than entities, so their `result` shape is based on the incoming DTO.
