...
Intercept decorated service execution around persistence work and confirmed transaction completion:

- Pre-process data before saving
- Post-process data after retrieval
- Database-level validations
- Data enrichment
- Post-commit and post-rollback side effects
...
3. Request transformation, validation, and typed query parsing
4. Optional generated-route transaction and request relation hydration
5. Decorated service lifecycle: UPDATE's GET, function `onBefore...`, repository work, and function `onAfter...`
6. A function-owned transaction commits plus `onAfterCommit` before the service returns; a joined route/scope owner remains open
7. Generated response reload when applicable, inside the boundary only for a route owner
8. A route-owned transaction commits plus `onAfterCommit`; a joined scope owner commits when its outer callback completes
9. Response transformation and route `onAfter...`
10. Authorization result handling, serialization, and **response**

Operation failures invoke matching error hooks. An owning boundary performs confirmed rollback plus `onAfterRollback`; a post-commit hook failure occurs after successful database commit and is reported separately rather than as rollback. See the lifecycle guide for operation-specific branches.
