...
Base class for RBAC policies. Provides the helper methods (`allow`, `deny`, `allowForRoles`, `scopeToOwner`) and the `getRulesForAction` dispatcher that normalizes hook output.
