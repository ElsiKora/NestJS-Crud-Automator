...
Base class for authorization policies. Provides helper methods (`allow`, `deny`, `allowForRoles`, `allowForPermissions`, `denyForPermissions`, `scopeToOwner`) for constructing rules that are executed by the policy executor.
