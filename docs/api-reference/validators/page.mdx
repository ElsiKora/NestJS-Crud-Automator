...
Property decorator factory that validates an object against one of several discriminator-backed schemas. For generated DTOs, prefer `ApiPropertyObject({ type: [...], discriminator, shouldValidateNested: true })`, which wires this validator for object payloads.
...
class PaymentDto {
	...
	payload!: CardPaymentDto | PayPalPaymentDto;
}
```

**Behavior:**

- The object must include the configured discriminator property
- The discriminator value must match one configured mapping entry
- The object must match the selected schema

## Paired Field Validators

### HasPairedCustomSuffixesFieldsValidator

Validates bracketed filter field groups such as `name[operator]`, `name[value]`, and `name[values]`.
...
- A `field[value]` or `field[values]` suffix requires `field[operator]`
- Operators that require a single value reject array values
- Operators that require an array reject single values
- Null operators such as `isnull` and `notnull` reject value suffixes
