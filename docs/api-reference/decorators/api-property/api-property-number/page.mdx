...
- Preserves missing optional values as `undefined`/`null` instead of coercing them to `NaN`.
- Applies `IsInt`/`IsNumber`, `IsDivisibleBy`, presence guards, `IsArray`, `ArrayMinSize`, `ArrayMaxSize`, and `ArrayNotEmpty` as needed.
