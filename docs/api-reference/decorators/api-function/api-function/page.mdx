...
| `persistenceMode`  | `EApiFunctionUpdatePersistenceMode` | UPDATE only: `SAVE` by default; explicit `PATCH` writes only supplied selected writable columns and requires `REQUIRED` or `MANDATORY`. |
