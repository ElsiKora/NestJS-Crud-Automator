...
Version `3.0.2` is the published baseline immediately before the additive generated-read work documented here. Package versions remain owned by release automation, so pin the exact released version your application has verified. Follow [Migrating to 3.0](/docs/nestjs-crud-automator/guides/migrating-to-3-0) when upgrading a pre-3.0 consumer.
...
For GET_LIST, a client `orderBy` replaces `defaultOrder`; `tieBreakers` are then appended with duplicate fields removed. A UUID `id` can be a server-only final tie-breaker without becoming a client-sort field. Send explicit `page` and `limit` values for the native stable pagination contract.
