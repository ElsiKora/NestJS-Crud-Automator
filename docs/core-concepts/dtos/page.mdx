...
- Full wrapper mode: `dto.response` is a class that describes the whole list response.
- Item DTO mode: `dto.response` is `{ itemType }`, and the framework generates the list wrapper.
