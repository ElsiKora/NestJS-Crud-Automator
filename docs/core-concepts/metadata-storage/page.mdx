...
```typescript
MetadataStorage.getInstance().setMetadata("UserEntity", "username", "api-property-describe", {
	type: EApiPropertyDescribeType.STRING,
	description: "Username",
	// ... other properties
});
...
```typescript
const metadata = MetadataStorage.getInstance().getMetadata("UserEntity", "username", "api-property-describe");
...
```typescript
const allMetadata = MetadataStorage.getInstance().getAllMetadata("UserEntity");
