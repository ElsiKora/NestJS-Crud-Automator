...
- Generated CURSOR GET_LIST routes use this existing decorated method for their `limit + 1` window and opposite-direction `take: 1` probe; PAGE routes continue to use `getList`.
...
Use names other than `get`, `getList`, `getMany`, `update`, and `delete` for ordinary domain methods. Starting with the 4.0 capability boundary, generated controller routes invoke those reserved operations only when the exact function was produced for the same entity and function type by `@ApiService` or the matching built-in `@ApiFunction*` decorator. Undecorated overrides can still be called directly, but generated routes reject them before the Automator-managed route transaction or repository I/O. Prefer subscribers for generated CRUD lifecycle changes and `@ApiFunctionCustom` or a custom route for a distinct operation. See [Migrating to 4.0](/docs/nestjs-crud-automator/guides/migrating-to-4-0).
