...
This generates `GET /providers/:providerKey/games/:id` and `GET /providers/:providerKey/games`. `providerKey` is the external owner-path name; `providerId` is the described direct scalar entity field used in the database predicate. Every required scalar path parameter inherited from `path` must be mapped exactly once; wildcard and optional/grouped dynamic parameters fail at bootstrap. Duplicate/missing parameters, duplicate fields, unknown path names, relations, and objects also fail at bootstrap. On GET, the inherited path must not reuse the generated primary identity parameter name, and the scope mapping does not rename that identity.

The framework generates a route-local PARAMETERS DTO from the mapped field metadata under the external owner-path parameter name and emits matching Swagger metadata. Do not combine this read scope with a manual `dto[EApiDtoType.PARAMETERS]`; use `autoDto[PARAMETERS]` only for validators on the generated DTO.
