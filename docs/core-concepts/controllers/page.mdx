...
Generated CRUD routes automatically receive pluralized Swagger summaries and descriptions based on the controller `name` option, or the entity name when `name` is omitted. For example, `name: "User"` produces text such as `Creating \`Users\`` and `This method is used for creating \`Users\``.
