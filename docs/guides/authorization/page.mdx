...
import type { TApiAuthorizationPolicyBeforeDeleteResult, TApiAuthorizationPolicyBeforeGetResult, TApiAuthorizationPolicyCustomActionResult } from "@elsikora/nestjs-crud-automator";
...
export class UserAccessPolicy extends ApiAuthorizationPolicyBase<UserEntity> {
	onBeforeGet(): TApiAuthorizationPolicyBeforeGetResult<UserEntity> {
		return this.allow({
			scope: ({ subject }) => ({ where: { id: subject.id } }),
		});
	}

	onBeforeDelete(): TApiAuthorizationPolicyBeforeDeleteResult<UserEntity> {
		return this.deny({
	...
	getCustomActionRule(action: "promote"): TApiAuthorizationPolicyCustomActionResult<UserEntity, "promote"> {
		if (action === "promote") {
