...
Validates an object property against one of multiple discriminator-backed schemas. Prefer `ApiPropertyObject({ type: [...], discriminator, shouldValidateNested: true })` for generated DTO object payloads because it wires the validator from property metadata.
