...
Generated GET_LIST routes read `limit` and `page` from the query DTO and pass them to the service as TypeORM `take` and `skip`. There is no route-level `pagination` config in the current public contract; enforce project-specific defaults or maximums with request validators, interceptors, or a custom route/service method.
...
export class UserService extends ApiServiceBase<UserEntity> {
	...
	async getUsersAfter(cursor: string, limit: number = 20): Promise<{ items: UserEntity[]; nextCursor: string | null }> {
