...
describe("ApiControllerTransformData", () => {
	...
	it("throws when required dynamic data is missing", () => {
		...
		const transformers = {
			[EApiControllerRequestTarget.BODY]: {
				transformers: [{ key: "signature", type: EApiControllerRequestTransformerType.DYNAMIC, value: TRANSFORMER_VALUE_DTO_CONSTANT.REQUEST_SIGNATURE }],
			},
		};
	...
	it("applies transformers to get list response data", () => {
		...
		const transformers = {
			[EApiControllerResponseTarget.RESPONSE]: {
				transformers: [{ key: "totalCount", type: EApiControllerRequestTransformerType.STATIC, value: "5" }],
			},
		};
	...
	it("throws for invalid dynamic transformer values", () => {
		...
		const transformers = {
			[EApiControllerRequestTarget.BODY]: {
				transformers: [{ key: "name", type: EApiControllerRequestTransformerType.DYNAMIC, value: "INVALID" }],
			},
		};
	...
	it("applies transformers to get list query data", () => {
		...
		const transformers = {
			[EApiControllerRequestTarget.QUERY]: {
				transformers: [{ key: "page", type: EApiControllerRequestTransformerType.STATIC, value: "2" }],
			},
		};
	...
	it("sets values even when keys are missing if configured", () => {
		...
		const transformers = {
			[EApiControllerRequestTarget.BODY]: {
				transformers: [{ key: "extra", type: EApiControllerRequestTransformerType.STATIC, value: "added", shouldSetValueEvenIfMissing: true }],
			},
		};
	...
	it("throws when key is missing and not forced", () => {
		...
		const transformers = {
			[EApiControllerRequestTarget.BODY]: {
				transformers: [{ key: "missing", type: EApiControllerRequestTransformerType.STATIC, value: "x" }],
			},
		};
	...
	it("applies transformers to request parameters", () => {
		...
		const transformers = {
			[EApiControllerRequestTarget.PARAMETERS]: {
				transformers: [{ key: "id", type: EApiControllerRequestTransformerType.STATIC, value: "2" }],
			},
		};
	...
	it("throws when authorized entity is missing", () => {
		...
		const transformers = {
			[EApiControllerRequestTarget.BODY]: {
				transformers: [{ key: "user", type: EApiControllerRequestTransformerType.DYNAMIC, value: TRANSFORMER_VALUE_DTO_CONSTANT.AUTHORIZED_ENTITY }],
			},
		};
